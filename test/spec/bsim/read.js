...
describe("bsim-moddle - read", function () {
  ...
  function fromFile(file, root, opts) {
    ...
    return createModdle(contents, root, opts);
  }
  ...
  describe("should import types", function () {
    ...
    describe("bpmn", function () {
      it("AdminProcess", async function (done) {
        // given
        console.log(moddle.getTypeDescriptor("bsim:definitions"));
        // given
        var xml = readFile("test/fixtures/xml/adminProcess_combined.bpmn");

        // when
        moddle.fromXML(xml, "bpmn:Definitions", function (err, proc) {
          console.log(proc);
          console.log("lalala", proc.rootElements);
            ...
            simulationConfiguration: [
              {
                startEvent: {
                  ...
                  arrivalRate: {
                    ...
                    distribution: {
                      $type: "bsim:exponentialDistribution",
                      mean: {
                        $type: "bsim:mean",
                        value: "150.5",
                      },
                ...
                endDateTime: "2016-08-05T15:00:00.000+02:00",
                gateways: [
                  {
                    $type: "bsim:exclusiveGateway",
                    id: "ExclusiveGateway_0a6d35t",
                    outgoingSequenceFlow: [
                      ...
                      },
                      {
                        ...
                        id: "SequenceFlow_1dvtw6t",
                      },
                    ],
                  },
                ],
                id: "SimulationConfiguration_2",
                processInstances: 30,
                processRef: "Process_2",
                startDateTime: "2016-08-01T09:00:00.000+02:00",
              },
            ],
          };
      ...
      it("Read Global Config File", async function () {
        // given

        // when
        var { rootElement } = await fromFile(
          "test/fixtures/xml/adminProcess_combined.bpmn",
          "bsim:globalConfiguration"
        );

        var expected = {
          $type: "bsim:globalConfiguration",
          id: "evaluationProcessA_sim",
          randomSeed: 3234,
          resourceData: {
            $type: "bsim:resourceData",
            dynamicResource: [
              {
      ...
      it("Read Global Config File Retailer", async function () {
        // given

        // when
        var { rootElement } = await fromFile(
          "test/fixtures/bsim/retailer_global.xml",
          "bsim:globalConfiguration"
        );
      ...
      it("Read SIM Config Retailer Batch SIM", async function () {
        // given

        // when
        var { rootElement } = await fromFile(
          "test/fixtures/bsim/retailer_batch_sim.xml"
        );
      ...
      it("CompensateEventDefinition", async function () {
        // when
        var { rootElement } = await fromFile(
          "test/fixtures/bpmn/compensate-event-definition.part.bpmn",
      ...
      it("SubProcess#incoming", async function () {
        ...
        // when
        var { rootElement } = await fromFile(
          "test/fixtures/bpmn/subprocess-flow-nodes-outgoing.part.bpmn",
          "bpmn:Process"
        );
        ...
        var expected = {
          $type: "bpmn:Process",
          flowElements: [expectedSubProcess, expectedSequenceFlow],
        };
      ...
      it("TimerEventDefinition#expression", async function () {
        ...
        // when
        var { rootElement } = await fromFile(file, "bpmn:TimerEventDefinition");
      ...
      it("Documentation", async function () {
        // when
        var { rootElement } = await fromFile(
          "test/fixtures/bpmn/documentation.bpmn"
        );
      ...
      it("ThrowEvent#dataInputAssociations", async function () {
        ...
        // when
        var { rootElement } = await fromFile(
          "test/fixtures/bpmn/throw-event-dataInputAssociations.part.bpmn",
          "bpmn:EndEvent"
        );
      ...
      it("Escalation + Error", async function () {
        // when
        var { rootElement } = await fromFile(
          "test/fixtures/bpmn/escalation-error.bpmn"
        );
      ...
      it("ExtensionElements", async function () {
        // when
        var { rootElement } = await fromFile(
          "test/fixtures/bpmn/extension-elements.bpmn"
        );
      ...
      it("ScriptTask", async function () {
        ...
        // when
        var { rootElement } = await fromFile(
          "test/fixtures/bpmn/scriptTask-script.part.bpmn",
          "bpmn:ScriptTask"
        );
      ...
      it("ItemDefinition#structureRef", async function () {
        // when
        var { rootElement } = await fromFile(
          "test/fixtures/bpmn/itemDefinition-structureRef.part.bpmn",
          "bpmn:ItemDefinition"
        );
      ...
      it("Operation#implementationRef", async function () {
        // when
        var { rootElement } = await fromFile(
          "test/fixtures/bpmn/operation-implementationRef.part.bpmn",
          "bpmn:Operation"
        );
      ...
      it("Interface#implementationRef", async function () {
        // when
        var { rootElement } = await fromFile(
          "test/fixtures/bpmn/interface-implementationRef.part.bpmn",
          "bpmn:Interface"
        );
      ...
      it("Lane#childLaneSet", async function () {
        // when
        var { rootElement } = await fromFile(
          "test/fixtures/bpmn/lane-childLaneSets.part.bpmn",
          "bpmn:Lane"
        );
      ...
      it("SequenceFlow#conditionExpression", async function () {
        // when
        var { rootElement } = await fromFile(
          "test/fixtures/bpmn/sequenceFlow-conditionExpression.part.bpmn",
          "bpmn:SequenceFlow"
        );
      ...
      it("MultiInstanceLoopCharacteristics#completionCondition", async function () {
        // when
        var { rootElement } = await fromFile(
          "test/fixtures/bpmn/multiInstanceLoopCharacteristics-completionCondition.part.bpmn",
      ...
      it("Association#associationDirection", async function () {
        // when
        var { rootElement } = await fromFile(
          "test/fixtures/bpmn/association.part.bpmn",
          "bpmn:Association"
        );
